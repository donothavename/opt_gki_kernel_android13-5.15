...
#include "../common/compiler.h" /* ZSTDH_ALIGNOF */
#include "../common/mem.h" /* S64 */
#include "../common/zstd_deps.h" /* ZSTDH_memset */
#include "../common/zstd_internal.h" /* ZSTDH_STATIC_ASSERT */
#include "hist.h" /* HISTH_add */
...
{
    ZSTDH_memset(fpstats, 0, sizeof(FPStats));
}
...
{
    ZSTDH_memset(fp, 0, sizeof(unsigned) * ((size_t)1 << hashLog));
    fp->nbEvents = 0;
...
#define FP_RECORD(_rate) ZSTDH_recordFingerprint_##_rate

#define ZSTDH_GEN_RECORD_FINGERPRINT(_rate, _hSize)                                 \
...
ZSTDH_GEN_RECORD_FINGERPRINT(1, 10)
ZSTDH_GEN_RECORD_FINGERPRINT(5, 10)
ZSTDH_GEN_RECORD_FINGERPRINT(11, 9)
ZSTDH_GEN_RECORD_FINGERPRINT(43, 8)
...
static void flushEvents(FPStats* fpstats)
{
    ...
    ZSTDH_memset(&fpstats->newEvents, 0, sizeof(fpstats->newEvents));
}
...
static size_t ZSTDH_splitBlock_byChunks(const void* blockStart, size_t blockSize,
                        int level,
                        void* workspace, size_t wkspSize)
{
    ...
    assert((size_t)workspace % ZSTDH_ALIGNOF(FPStats) == 0);
    ZSTDH_STATIC_ASSERT(ZSTDH_SLIPBLOCK_WORKSPACESIZE >= sizeof(FPStats));
...
/* ZSTDH_splitBlock_fromBorders(): very fast strategy :
 ...
 */
static size_t ZSTDH_splitBlock_fromBorders(const void* blockStart, size_t blockSize,
                        void* workspace, size_t wkspSize)
{
#define SEGMENT_SIZE 512
    ...
    initStats(fpstats);
    HISTH_add(fpstats->pastEvents.events, blockStart, SEGMENT_SIZE);
    HISTH_add(fpstats->newEvents.events, (const char*)blockStart + blockSize - SEGMENT_SIZE, SEGMENT_SIZE);
    ...
    HISTH_add(middleEvents->events, (const char*)blockStart + blockSize/2 - SEGMENT_SIZE/2, SEGMENT_SIZE);
...
size_t ZSTDH_splitBlock(const void* blockStart, size_t blockSize,
                    int level,
                    void* workspace, size_t wkspSize)
{
    DEBUGLOG(6, "ZSTDH_splitBlock (level=%i)", level);
    assert(0<=level && level<=4);
    if (level == 0)
        return ZSTDH_splitBlock_fromBorders(blockStart, blockSize, workspace, wkspSize);
    /* level >= 1*/
    return ZSTDH_splitBlock_byChunks(blockStart, blockSize, level-1, workspace, wkspSize);
}
