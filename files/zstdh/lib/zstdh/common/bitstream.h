...
MEM_STATIC size_t BIT_initDStream(BIT_DStream_t* bitD, const void* srcBuffer, size_t srcSize)
{
    if (srcSize < 1) { ZSTDH_memset(bitD, 0, sizeof(*bitD)); return ERROR(srcSize_wrong); }
    ...
    if (srcSize >=  sizeof(bitD->bitContainer)) {  /* normal case */
        ...
        { BYTE const lastByte = ((const BYTE*)srcBuffer)[srcSize-1];
          bitD->bitsConsumed = lastByte ? 8 - ZSTDH_highbit32(lastByte) : 0;  /* ensures bitsConsumed is always set */
        ...
        {   BYTE const lastByte = ((const BYTE*)srcBuffer)[srcSize-1];
            bitD->bitsConsumed = lastByte ? 8 - ZSTDH_highbit32(lastByte) : 0;
